...
When ``bash`` and ``ksh`` users update their environment with ``setup-env.sh``, it will check for spack-installed environment modules or lmod and add the ``module`` command to their environment; This only occurs if the module command is not already available. You can install ``environment-modules`` and ``lmod`` with ``spack bootstrap`` as described in :ref:`ModuleSystems`.
